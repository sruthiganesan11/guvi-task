...
// coverting the json data into table
...
// Function to update the table based on the current step
const updateTable = () => {
  const start = currentStep * tableSize;
  ...
  // Populate the table with the data to display
...
startBtn.addEventListener("click", initializePagination);

endBtn.addEventListener("click", () => {
  currentStep = maxIndex - 1;
  updateTable();
  updateBtn();
});

// Initial pagination initialization
